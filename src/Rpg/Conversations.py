...
class Conversations:
    ...
    def __init__(self, conv, lang):
        self.conversation = self.generate_conversation(conv, lang)
        self.str_counter = 0
        self.str_list = []
        return
    ...
    def get_next_string(self, id):
        if self.str_counter == 0:
            self.str_list = self.conversation.get_texts_by_id(id)
        str = self.str_list[self.str_counter]
        if self.str_counter+1 >= len(self.str_list):
            self.str_counter = 0
            self.str_list = []
        else:
            self.str_counter +=1
